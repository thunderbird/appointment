...
test.describe('set availability', {
  ...
  test('able to turn off availability', async ({ page }) => {
    ...
    await expect(availabilityPage.bookableToggle).toBeChecked({ checked: false });

    // go to share link/book appointment page and verify not available
    await bookApptPage.gotoBookingPageShortUrl();
    await page.waitForTimeout(TIMEOUT_1_SECOND);
    await expect(bookApptPage.scheduleTurnedOffText).toBeVisible();
