...
test.describe('general settings', {
  ...
  test('able to change language', async ({ page }) => {
    // change language setting to DE and verify
    await settingsPage.changeLanguageSetting(APPT_LANGUAGE_SETTING_DE);
    await expect(settingsPage.settingsHeaderDE).toBeVisible({ timeout: 15000 });
    await expect(settingsPage.generalSettingsHeaderDE).toBeVisible();
  ...
  test('able to change time format', async ({ page }) => {
    ...
    await expect(dashboardPage.calendarEvent24hrFormat).toBeVisible({ timeout: 30000 });
    await settingsPage.gotoGeneralSettingsPage();
  ...
  test('able to change timezone', async ({ page }) => {
    ...
    await dashboardPage.gotoToDashboardMonthView();
    await expect(dashboardPage.timezoneDisplayTextHalifax).toBeVisible({ timeout: 30000 });
