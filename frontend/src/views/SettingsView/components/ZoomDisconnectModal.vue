...
async function disconnect() {
  await externalConnectionStore.disconnect(ExternalConnectionProviders.Zoom, props.typeId);
...
<template>
  ...
  <modal-dialog ref="modal" class="zoom-disconnect-modal">
    <template #header>
      {{ t('text.settings.connectedApplications.disconnect.zoom.title') }}
    </template>

    <div class="zoom-disconnect-modal-container">
      ...
      {{ t('text.settings.connectedApplications.disconnect.zoom.message') }}
    </div>
    ...
    <template #actions>
      <primary-button
        ...
        @click="modal?.hide()"
        class="cancel-button"
        data-testid="zoom-disconnect-cancel-btn"
      >
        {{ t('text.settings.connectedApplications.disconnect.zoom.cancel') }}
      </primary-button>
      <danger-button
        name="disconnect"
        @click="disconnect()"
        class="disconnect-button"
        data-testid="zoom-disconnect-confirm-btn"
      >
        {{ t('text.settings.connectedApplications.disconnect.zoom.confirm') }}
      </danger-button>
    </template>
  </modal-dialog>
</template>
