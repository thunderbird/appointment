...
const isExpired = computed(() => {
  return (
    props.appointment?.slots.reduce((p, c) => dj.max(p, dj(c.start).add(c.duration, 'minutes')), dj('1970-01-01')) <
    dj()
  );
});
...
<template>
  ...
  >
    ...
    <template
      #title
      v-if="appointment && panelStep === APPOINTMENT_SLIDING_PANEL_STEPS.MODIFY && !hideModifyFieldsAndCTA"
    >
      <text-input name="appointmentTitle" v-model="appointmentTitle" :placeholder="appointment.title" />
    </template>
    ...
    <template #cta v-if="panelStep === APPOINTMENT_SLIDING_PANEL_STEPS.DETAILS">
      ...
      <div
        v-else-if="isExpired || status === BookingStatus.Cancelled || status === BookingStatus.Declined"
        class="cta-single"
      >
      ...
      <div v-else-if="status === BookingStatus.Requested" class="cta-dual">
        <primary-button class="btn-confirm" @click="moveToConfirmAppointmentStep(true)" :title="t('label.confirm')">
          {{ t('label.confirmBooking') }}
        </primary-button>
        <danger-button class="btn-deny" @click="moveToConfirmAppointmentStep(false)" :title="t('label.deny')">
          {{ t('label.denyBooking') }}
        </danger-button>
      </div>
    </template>
