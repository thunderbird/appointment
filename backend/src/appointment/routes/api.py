...
def create_my_calendar(
    ...
    # HERE: the frontend will probably need to specify the external connection id
    # this means that you'll need to update the schema
...
def read_remote_calendars(
    ...
    # HERE: the frontend will probably need to specify the external connection
    # this means that you'll need to update the schema
...
def sync_remote_calendars(
    ...
    # HERE: You'll get *all* the external connections so you can loop through each of them
...
def read_remote_events(
    ...
    if db_calendar.provider == CalendarProvider.google:
        # HERE: You'll want to use db_calendar.external_connection_id
        # CHECK: or...just db_calendar.external_connection? can't i just use it as a property instead of fetching?
        external_connection = utils.list_first(
            repo.external_connection.get_by_type(db, subscriber.id, schemas.ExternalConnectionType.google)
        )
