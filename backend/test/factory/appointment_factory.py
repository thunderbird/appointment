    ...
    def _make_appointment(calendar_id=FAKER_RANDOM_VALUE,
        ...
        with with_db() as db:
            appointment = repo.create_calendar_appointment(db, schemas.AppointmentFull(
                ...
                slug=slug if factory_has_value(slug) else fake.slug(),
                ...
                location_phone=location_phone if factory_has_value(location_phone) else fake.phone(),
                keep_open=keep_open,
                status=status,
                meeting_link_provider=meeting_link_provider,
            ), calendar_id if calendar_id else make_caldav_calendar().id)

            if factory_has_value(slots):
